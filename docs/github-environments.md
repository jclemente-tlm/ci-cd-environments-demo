...
| `qa-signoff` | No aplica | No requerido | Manual después de pruebas funcionales | Required reviewers de QA |
...
3. En `dev`, `qa` y `prod`, agregar el secret `DEMO_DEPLOY_TOKEN` con un valor ficticio diferente. `qa-signoff` no necesita configuración de aplicación ni secrets.
4. En `qa-signoff`, habilitar **Required reviewers**, seleccionar al equipo QA, activar prevent self-review y restringir la rama a `main`.
